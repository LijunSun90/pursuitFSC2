...
class AECEnv:
    ...
    def last_vsos(self, agent_id, observe=True):
        """
        Added in Version-SOS.
        Return observation, cumulative reward, done, info   for the specified agent.
        """
        agent_id = "pursuer_" + str(agent_id)
        observation = self.observe(agent_id) if observe else None
        return observation, self._cumulative_rewards[agent_id], self.dones[agent_id], self.infos[agent_id]

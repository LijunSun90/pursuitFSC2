...
class Pursuit:
    ...
    def __init__(self, **kwargs):
        ...
        if self.flatten:
            obs_space = spaces.Box(self.low, self.high)
        else:
    ...
    def reset(self):
        ...
        return [self.collect_obs_by_idx(self.pursuer_layer, i) for i in range(self.num_agents)]

    def step_all(self, actions):
        """
            Added in SOS.
            Step the system forward. Actions is an iterable of action indices.
        """
        # move allies
        rewards = self.latest_reward_state
        ...
        obslist = [self.collect_obs_by_idx(self.pursuer_layer, i) for i in range(self.num_agents)]
        rewards = self.latest_reward_state
        # rewards = self._cumulative_rewards
    ...
    def step(self, action, agent_id, is_last):
        ...
        # The following line is comment only in Version-MADDPG for step_all().
        # self.latest_reward_state = [0 for _ in range(self.num_agents)]

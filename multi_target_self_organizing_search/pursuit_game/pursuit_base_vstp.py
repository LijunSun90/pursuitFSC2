...
# from .utils import agent_utils_vsop as agent_utils
from .utils import agent_utils_vstp as agent_utils
...
class Pursuit:
    ...
    def __init__(self, **kwargs):
        ...
        self.pursuers, self.evaders = agent_utils.create_agents_vsop(self.n_pursuers, self.n_evaders,
        ...
        # Modified in Version-STP. Original: kwargs.pop('tag_reward', 0.01).
        self.tag_reward = kwargs.pop('tag_reward', 0.1)
    ...
    def reward(self, agent_id):
        """
        This function is rewritten in Version-STP based on the original reward().
        """
        reward = self.move_reward
        # Added in Version-STP.
        es = self.evader_layer.get_state_matrix()  # evader positions
        reward += self.tag_reward * \
            np.sum(es[np.clip(self.pursuer_layer.get_position(agent_id)[0] + self.surround_mask[:, 0],
                              0, self.x_size - 1),
                      np.clip(self.pursuer_layer.get_position(agent_id)[1] + self.surround_mask[:, 1],
                              0, self.y_size - 1)])
    ...
    def get_collision_reward(self):
        ...
        # Added in V-STP.
        # collision_rewards += \
        #     self.collision_reward * np.asarray([pursuer.collide_with_obstacle for pursuer in self.pursuers])
    ...
    def get_coordination_reward(self, pursuers_who_capture=None):
        ...
        # Reward.
        shared_reward = \
            agent_utils.get_coordination_reward_v6(pursuers_positions, evaders_positions, self.obs_offset)

...
from actor_critic.common.logx import colorize
from actor_critic.common.mpi_tools import mpi_fork
...
def setup_logger_kwargs(exp_name, seed=None, data_dir=None, datestamp=False):
    """
    Set up the output_dir for a logger and returns a dict for logger kwargs.
    ...
    :param exp_name: String. Name for experiment.
    :param seed: Int.Seed for random number generators used by experiment.
    :param data_dir: Stirng. Path to folder where results should be saved.
        Default is the `DEFAULT_DATA_DIR` in `user_config`.
    :param datestamp: Boolean. Whether to include a date and timestep in the
        name fo the save directory.
    :return: Dict. logger_kwargs, containing output_dir and exp_name.
    """
    # Datestamp forcing.
...
def call_experiment(exp_name, thunk, seed=0, num_cpu=1, data_dir=None, datestamp=False, **kwargs):
    ...
    This wraps a few pieces of functionality which are useful when you want to
    run many experiments in sequence, including logger configuration and
    splitting into multiple processes for MPI.

    There's also a Spinningup-specific convenience added into executing the thunk:
    if `env_name` is one of the kwargs passed to call_experiment, it's assumed
    that the thunk accepts an argument called `env_fn`, and that the `env_fn`
    should make a gym environment with the given `env_name`.

    The way the experiment is actually executed is slightly complicated: the function
    is serialized to a string, and then `run_entrypoint.py` is executed in a subprocess
    call with the serialized string as an argument. `run_entrypoint.py` unserializes
    the function call and executes it. We choose to do it this way--instead of just
    calling the function directly here--to avoid leaking state between successive
    experiments.

    :param exp_name: String. Name for experiment.
    :param thunk: Callable. A python function.
    :param seed: Int. Seed for random number generators.
    :param num_cpu: Int. Number of MPI processes to split into. Also accepts 'auto',
        which will set up as many process as there are cpus on the machine.
    :param data_dir: String. Used in configuring the logger, to decide where to store
        experiment results. Note: if left as None, data_dir will default to
        `DEFAULT_DATA_DIR` from `user_config.py`.
    :param datestamp:
    ...
    picked_thunk = cloudpickle.dumps(thunk_plus())
    encoded_thunk = base64.b64encode(zlib.compress(picked_thunk)).decode('utf-8')
    ...
    try:
        ...
        err_msg = '\n' * 3 + '=' * DIV_LINE_WIDTH + '\n' + dedent("""
            ...
            There appears to have been error in your experiment.
...
def valid_str(v):
    ...
    # Valid characters are '-', '_', and alphanumeric. Replace invalid chars with '-'.
    str_v = str(v).lower()
...
class ExperimentGrid:
    ...
    def name(self, name):
        assert isinstance(name, str), "Name ahs to be a string."
        self._name = name
    ...
    def add(self, key, vals, shorthand=None, in_name=False):
        ...
        :param vals: value or list of values. Allowed values of parameter.
        :param shorthand: String. Optional, shortened name of parameter. For
            example, maybe the parameter ``steps_per_epoch`` is shortened to ``steps``.
        :param in_name: Boolean. When constructing variant names, force the
